...
export default class AIFEXPage {
    private _page : Page;
    ...
    async connectToSession(url : string) {
        if (this._extensionKey) {
            await this._page.goto(`chrome-extension://${this._extensionKey}/aifex_page/index.html`, {waitUntil: "domcontentloaded"});
            await this._page.waitForSelector('#connectionURLInput');
            const connexionInputHandler = await this._page.$('#connectionURLInput');
